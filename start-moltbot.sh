...
if (isWorkersAI) {
    ...
    config.agents.defaults.model.primary = process.env.MOLTBOT_MODEL || 'anthropic/claude-opus-4-5-20251101';
} else {
    // Default to Anthropic without custom base URL (uses built-in pi-ai catalog)
    config.agents.defaults.model.primary = process.env.MOLTBOT_MODEL || 'anthropic/claude-opus-4-5';
}
